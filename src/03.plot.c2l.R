...
c2ls = lapply(c2lnames, function(a){
  ...
  r$barcode = substr(rownames(r),1,18)
  #r = split(r,m[,2])
...
# plot tUMI ############
pdf(paste0('figures/tUMI.pdf'),w=4*3.5,h=3*3)
par(mfrow=c(3,4),mar=c(0.1,0.1,1.2,6),bty='n',oma=c(0,0,0,0))
for(n in names(vs)){
...
for(a in names(c2ls)){
  ...
  for(n in names(vs)){
    ...
    plotVisium(vs[[n]],vs[[n]]$nCount_Spatial,zfun = log1p,main=n,cex = scaleTo(log1p(vs[[n]]$nCount_Spatial)),legend.args = list(title='tUMI'))
    plotVisium(vsf[[n]],pie.fraqs=c2ls[[a]][[n]],pie.cols=ctcols,pie.min.fraq=0.01,main=a,cex = 1,plot.legend = F)
    plotVisium(vsf[[n]],weightedColourMeans(ctcols,c2ls[[a]][[n]]))
...
for(a in names(c2ls)){
  ...
  for(ct in celltypes){
    for(sid in names(vsf)){
      plotVisium(vsf[[sid]],c2ls[[a]][[sid]][,ct],zfun = log1p,cex = scaleTo(log1p(c2ls[[a]][[sid]][,ct])),main=paste0(sid,'; ',ct))
    }
  }
  dev.off()
}
